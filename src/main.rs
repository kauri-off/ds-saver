...
fn main() {
    ...
    match opt {
        Opt::Chats { token } => {
            console::print_chats(get_chats(&token));
        },
        Opt::Messages { token, chat_id, output, print, full } => {
            let messages = get_messages(&chat_id, &token, full);
            if print {
            ...
            match output {
                ...
                None => {
                    if !print {
                        ...
                        println!("Messages saved to {}, check 'messages --help'", format!("{}.json", chat_id))
                    }
                },
            }
        },
...
enum Opt {
    ...
    Messages {
        ...
        #[structopt(short, long, about = "Print messages to screen, if not save to default path")]
        print: bool,
